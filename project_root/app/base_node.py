...
def get_knowledge_type(state):
    ...
    system_prompt = """
        ...
        """
    grade_messages = [SystemMessage(content=system_prompt), HumanMessage(content=f"{question}")]
...
def grade_documents(state):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            print(result)
            if result[0] == "yes":
                filter_documents.append(documents[result[1]])
    if grade_count:
        grade_count += 1
    else:
        grade_count = 1

    return {"question": question, "documents": filter_documents, "filename": filename, "grade_count": grade_count}
