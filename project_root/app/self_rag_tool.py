...
class GradeAndGenerateTool(object):
    ...
    def grade(self, question, text, idx):
        system_prompt = """
                ...
                """
        grade_messages = [SystemMessage(content=system_prompt), HumanMessage(content=f"问题：{question}\n文档：{text}")]
    ...
    def rewrite_question(self, question):
        rewrite_prompt = "您是一个将输入问题转换为优化的更好版本的问题重写器\n用于矢量库检索。查看输入并尝试推理潜在的语义意图/含义。"
        rewrite_prompt_messages = [SystemMessage(content=rewrite_prompt), HumanMessage(content=f"问题：{question}")]
        result = self.llm.invoke(rewrite_prompt_messages)
        return result.content
